...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	const fanfiction = await locals.pb
		.collection('fanfictions')
		.getOne<FanfictionsResponse<{ author: UsersResponse }>>(id, { expand: 'author' });
	...
	const favs = await locals.pb
		.collection('fanfiction_favorites')
		.getFullList<FanfictionFavoritesResponse>({
			filter: `target="${id}"`
		});
	const user = locals.auth;
	const alreadyFaved = user == null ? false : await hasUserAlreadyFaved(user.id, fanfiction.id);
	const isVerified = locals.isVerified;
	return { fanfiction, favs, user, alreadyFaved, comments, isVerified, language: getLocale() };
};
...
export const actions = {
	...
	removeFavorite: async (event) => {
		...
		try {
			const response = await event.locals.pb
				.collection('fanfiction_favorites')
				.getFullList<FanfictionFavoritesResponse>({
